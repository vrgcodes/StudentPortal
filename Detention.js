...
app
        ...
        connection.query(sql, async (err, results)=>{
            ...
            if(results.length===0){
                ...
                if (isMatch && results[0].Status !=="TBD"){ //Only if the the login details match and the user has been approved the following is performed
                    ...
                    res.json({match:true, accessToken:accessToken, refreshToken:refreshToken}); //Sends the following data back to the client-side JS
                }
                else{
                    res.json({match:false})
                }
    ...
    .get('/viewUniform', (req, res)=>{ //Viewing uniform violations handler
        ...
        connection.query(sql, (err, results)=>{
            ...
            const columnWidths = [100, 100, 130, 110]; 
    ...
    .get('/viewDetention', (req, res)=>{
        ...
        connection.query(sql, (err, results)=>{
            ...
            const tableRows = [headers].concat(results.map(row => [formatDate(row.Date), row.Name, row.Class, row.Reason, row.Prefect_Teacher]));
            const columnWidths = [100, 70, 100, 85, 100]; 
            const startY = doc.y + 10;
            const startX = doc.x + 10;
